...
=====================================================
VERSION 2 (ABLATION MODEL - Ratio Features) + RANKNET
"""
...
CONFIG_ID = 32            # Cấu hình trong sổ N
N_CONFIGS_TESTED = 32    
...
CB_DRAWDOWN_THRESHOLD = -0.15      # Crisis Trigger: Rất nhạy (để test bão)
CB_DRAWDOWN_THRESHOLD_WF = -0.35   # Dài hạn: Cắt lỗ ở -25% (hợp lý với chứng khoán VN)
CB_DESIGN_CONTRACT = -0.40        # Lời hứa: Tuyệt đối không để lỗ quá -30% (Headroom = 5%)
CB_RECOVERY_VOL_QUANTILE = 0.5 # v1: 0.8 → v2: 0.5->base: 0.4
CORRELATION_MAX = 0.7
...
LR = 0.001
BATCH_SIZE = 32 # Không còn ý nghĩa cho training (do batch = date), nhưng giữ để log hoặc predict
MAX_EPOCHS = 200
...
def add_vnindex_context(df):
    """Ghép return VNINDEX (lag 1 ngày) vào mọi mã theo ngày — feature cấp thị trường."""
    vni = df[df['ticker'] == 'VNINDEX'][['date', 'close']].copy()
    ...
    vni = vni[['date', 'vnindex_return_1d_lag_1']]
    return df.merge(vni, on='date', how='left')
...
def prepare_training_data_by_date(df_features, tickers, train_dates, sequence_len=SEQUENCE_LENGTH):
    """
    Chuẩn bị dữ liệu theo từng ngày.
    Mỗi ngày sẽ là một batch chứa tất cả các mã hợp lệ.
    Loại bỏ các ngày có ít hơn 2 mã.
    """
    sequences = {}
    for tic in tickers:
        ...
        for i in range(sequence_len, len(sub)):
            date = arr_d[i]
            tic_seqs[date] = (arr_f[i-sequence_len:i], arr_t[i])
        if tic_seqs:
            sequences[tic] = tic_seqs
            
    dates_dict = {}
    for tic, tic_seqs in sequences.items():
        for date, (x, y) in tic_seqs.items():
            if date not in dates_dict:
                dates_dict[date] = {'X': [], 'y': []}
            dates_dict[date]['X'].append(x)
            dates_dict[date]['y'].append(y)
    ...
    for date in sorted(dates_dict.keys()):
        if len(dates_dict[date]['y']) >= 2:
            X_grouped.append(np.array(dates_dict[date]['X']))
            y_grouped.append(np.array(dates_dict[date]['y']).reshape(-1, 1))
            valid_dates.append(date)
            
    return X_grouped, y_grouped, np.array(valid_dates)

def pairwise_ranking_loss(y_true, y_pred, margin=0.0):
    """
    Tính pairwise ranking loss cho một batch (các mã trong cùng 1 ngày).
    """
    y_true_diff = y_true - tf.transpose(y_true)
    y_pred_diff = y_pred - tf.transpose(y_pred)
    
    # mask: chỉ xét các cặp (i, j) mà y_true[i] > y_true[j]
    mask = tf.cast(y_true_diff > 0, dtype=tf.float32)
    
    loss_matrix = tf.nn.relu(margin - y_pred_diff) * mask
    
    num_valid_pairs = tf.reduce_sum(mask)
    loss = tf.reduce_sum(loss_matrix) / tf.maximum(num_valid_pairs, 1.0)
    return loss

def train_model_custom(model, X_grouped, y_grouped, sample_dates, split_date, checkpoint_path=None):
    train_idx = [i for i, d in enumerate(sample_dates) if d < split_date]
    val_idx = [i for i, d in enumerate(sample_dates) if d >= split_date]
    ...
    X_reshaped = X_train_concat.reshape(-1, X_train_concat.shape[-1])
    scaler = StandardScaler()
    scaler.fit(X_reshaped)
    
    if checkpoint_path and os.path.exists(checkpoint_path):
        print(f"    [RESUME] Tải weights từ checkpoint: {checkpoint_path}")
        model.load_weights(checkpoint_path)
        return model, scaler
    
    def scale_batch(batch):
        shape = batch.shape
        scaled = scaler.transform(batch.reshape(-1, shape[-1])).reshape(shape)
        return tf.convert_to_tensor(scaled, dtype=tf.float32)
        
    X_train_scaled = [scale_batch(X_grouped[i]) for i in train_idx]
    y_train_tensors = [tf.convert_to_tensor(y_grouped[i], dtype=tf.float32) for i in train_idx]
    
    X_val_scaled = [scale_batch(X_grouped[i]) for i in val_idx]
    y_val_tensors = [tf.convert_to_tensor(y_grouped[i], dtype=tf.float32) for i in val_idx]
    ...
    feature_dim = X_train_scaled[0].shape[-1]
    
    @tf.function(input_signature=[
        tf.TensorSpec(shape=[None, SEQUENCE_LENGTH, feature_dim], dtype=tf.float32),
        tf.TensorSpec(shape=[None, 1], dtype=tf.float32)
    ])
    def train_step(x_batch, y_batch):
        with tf.GradientTape() as tape:
            y_pred = model(x_batch, training=True)
            loss_value = pairwise_ranking_loss(y_batch, y_pred, margin=RANKING_MARGIN)
            if len(model.losses) > 0:
                loss_value += tf.add_n(model.losses)
        grads = tape.gradient(loss_value, model.trainable_weights)
        grads, _ = tf.clip_by_global_norm(grads, 5.0)
        optimizer.apply_gradients(zip(grads, model.trainable_weights))
        num_pairs = tf.reduce_sum(tf.cast(y_batch - tf.transpose(y_batch) > 0, tf.float32))
        return loss_value, num_pairs
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for idx in indices:
            loss_val, num_pairs = train_step(X_train_scaled[idx], y_train_tensors[idx])
            if num_pairs > 0:
                train_loss_sum += float(loss_val)
                valid_train_batches += 1
                
        avg_train_loss = train_loss_sum / valid_train_batches if valid_train_batches > 0 else 0.0
        
        val_loss_sum = 0.0
        valid_val_batches = 0
        for x_batch, y_batch in zip(X_val_scaled, y_val_tensors):
            loss_val, num_pairs = val_step(x_batch, y_batch)
            if num_pairs > 0:
                val_loss_sum += float(loss_val)
                valid_val_batches += 1
                
        avg_val_loss = val_loss_sum / valid_val_batches if valid_val_batches > 0 else 0.0
        
        print(f"    Epoch {epoch+1:03d} - train_loss: {avg_train_loss:.6f} - val_loss: {avg_val_loss:.6f}")
        ...
        if patience_counter >= EARLY_STOP_PATIENCE:
            print(f"    Early stopping at epoch {epoch+1} with best val_loss = {best_val_loss:.6f}")
            break
    ...
    if checkpoint_path:
        ...
        print(f"    [CHECKPOINT] Đã lưu model tốt nhất tại: {checkpoint_path}")
...
def build_lstm(input_shape):
    ...
    model.add(Dropout(DROPOUT))
    model.add(Dense(1))
    # We won't use compile with loss='mse' since we use custom loop
    ...
    for i, date in enumerate(sim_days):
        ...
        if not in_cash:
            ...
            if drawdown <= cb_threshold:
                print(f"    [DEBUG-FEE] {date.date()}: peak_value={peak_value:.6f}, "
                      f"current_value_pre_fee={current_value:.6f}, "
                      f"prev_value_hom_qua={prev_value_hom_qua:.6f}")
                ...
                print(f"    [DEBUG-FEE] fee_cost={fee_cost:.6f}, current_value_post_fee={current_value:.6f}, "
                      f"dd_post_fee_vs_peak={(current_value/peak_value - 1):.6f}")
                print(f"    [DEBUG-FEE] >>> Kỳ vọng Last Day DD cho episode kết thúc {date.date()} ≈ {(current_value/peak_value - 1):.4f}")
...
def compute_expected_mdd(sigma_daily, T_days, mu_daily=0.0):
    ...
    if sigma_daily > 0:
        drift_ratio = mu_daily / (sigma_daily ** 2)  
    else:
...
def evaluate_crisis_fold(df_features, tickers_all, trading_days, train_end_marker, test_start_marker,
    ...
    checkpoint_path = f"path/ranknet/checkpoints/ranknet_crisis_{fold_name}.weights.h5"
    model, scaler = train_model_custom(model, X_grouped, y_grouped, sample_dates, split_date, checkpoint_path=checkpoint_path)
...
def main():
    print("=" * 70)
    print("PIPELINE ABLATION: V1 Features/LSTM + V2 Risk Mgmt + RANKNET")
    print("=" * 70)
    ...
    print(f"  Loss: Pairwise Ranking Loss (RankNet) - Margin {RANKING_MARGIN}")
    ...
    if datetime.now() > datetime(2026, 3, 31):
        print("\n*** CẢNH BÁO: Crisis fold 2026 đang chạy post-hoc (dữ liệu Q1/2026 đã tồn tại). Xác nhận rằng fold này đã được pre-register trước ngày 2026-01-01 để đảm bảo OOS hợp lệ. ***")
    ...
    print("\n===== BẮT ĐẦU 5-FOLD WALK-FORWARD LIÊN TỤC (ABLATION + RANKNET) =====")
    fold_results = []
    current_state = None
    ...
    for i, (train_marker, test_start_marker, test_end_marker) in enumerate(FOLDS_5):
        ...
        print(f"  Training samples: {sum(len(x) for x in X_grouped)} (grouped into {len(X_grouped)} daily batches), features: {X_grouped[0].shape[-1]}")
        ...
        checkpoint_path = f"path/ranknet/checkpoints/ranknet_fold_{i+1}.weights.h5"
        model, scaler = train_model_custom(model, X_grouped, y_grouped, sample_dates, split_date, checkpoint_path=checkpoint_path)
    ...
    for fold_idx, fold_mdd in enumerate(fold_mdds):
        marker = "⚠️" if fold_mdd < -0.15 else "  "
