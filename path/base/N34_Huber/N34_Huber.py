...
=====================================================
VERSION 2 (ABLATION MODEL - Ratio Features) — Sử dụng Ratio Features V2 / Giữ nguyên LSTM V1:
  1. Feature engineering: Sử dụng bộ Ratio Features từ V2 (loại bỏ giá tuyệt đối)
  2. Model: Giữ V1 (LSTM 3×64, ~85K params, sequence 20)
  3. Risk management: dual CB (-10% crisis / -35% walk-forward),
...
Kết luận: Việc điều tra kỹ thuật (đo lường MDD, phí, DSR) đến đây có thể coi là
 hoàn tất — không còn lỗ hổng nào đang treo.
 Tuy nhiên, đối với nguyên nhân fail (đặc biệt là đợt khủng hoảng COVID), không nên
 gộp chung hoàn toàn vào "khả năng dự đoán của model kém". Cần ghi nhận rằng σ_stress
 thực tế trong đợt COVID tăng vọt quá mạnh so với ước lượng ex-ante, vượt khỏi
 khả năng chống chịu của hệ thống Circuit Breaker vốn được thiết kế cho điều kiện
 bình thường/stress vừa. Đây là yếu tố thị trường cực đoan (Black Swan) tác động
 trực tiếp, chứ không hoàn toàn do lỗi của model hay lớp quản trị rủi ro.
"""
...
CONFIG_ID = 10           # N34: Huber Loss
N_CONFIGS_TESTED = 34    # Đếm theo số lựa chọn thiết kế thực tế (features, LSTM arch, CB, vol quantiles, L2...)
...
def build_lstm(input_shape, loss_fn='mse'):
    model = Sequential()
...
def run_backtest(df_features, model, scaler, tickers_all, start_date, end_date, trading_days,
    ...
    for i, date in enumerate(sim_days):
        # Lưu lại prev_value gốc của ngày hôm qua trước bất kỳ cập nhật nào
...
def evaluate_crisis_fold(df_features, tickers_all, trading_days, train_end_marker, test_start_marker,
    ...
    X, y, sample_dates = prepare_training_data(train_df, tickers_all,
    ...
    delta_val = train_df['target'].std()
    print(f"   >>> Dùng Huber Loss (delta={delta_val:.5f}). Lưu ý: val_loss khác thang đo so với MSE, không thể so sánh trực tiếp.")
    huber_loss = tf.keras.losses.Huber(delta=delta_val)

    model = build_lstm((SEQUENCE_LENGTH, X.shape[2]), loss_fn=huber_loss)
...
def main():
    ...
    df_features = add_vnindex_context(df)   # merge trước
    df_features = add_features(df_features)  # rồi tính 13 feature cũ như bình thường
    ...
    for i, (train_marker, test_start_marker, test_end_marker) in enumerate(FOLDS_5):
        ...
        delta_val = train_df['target'].std()
        print(f"  >>> Dùng Huber Loss (delta={delta_val:.5f}). Lưu ý: val_loss (Huber) không so sánh trực tiếp với MSE của các N trước.")
        huber_loss = tf.keras.losses.Huber(delta=delta_val)
    ...
    if binding_psr >= PSR_THRESHOLD and psr_model > psr_baseline and covid_passed:
        ...
        print(f"    Nguyên nhân: Mặc dù model có thể chưa đủ tốt, nhưng cần lưu ý σ_stress trong đợt COVID quá cao")
        print(f"                 so với ước lượng ex-ante, dẫn đến CB kích hoạt sớm. Đây là yếu tố thị trường cực đoan.")
    else:
        print(f">>> KẾT LUẬN: Mô hình Ablation KHÔNG đạt tín hiệu.")
        ...
        print(f"    Lưu ý: Nếu fail chủ yếu ở crisis fold, cần xét yếu tố σ_stress thực tế (như COVID) quá cao.")
