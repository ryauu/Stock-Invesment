...
=================================================================
VERSION 2 — ĐỢT 1 SỬA TOÀN DIỆN:
  1. Feature engineering: loại bỏ giá tuyệt đối, chỉ dùng ratio/return + RSI-14
  2. Model: LSTM 1×32 (~5.5K params) + L2 regularization, sequence 10
  3. Risk management: dual CB (-20% crisis / -35% walk-forward),
     vol quantile 0.75, recovery nhanh hơn
  4. Explicit FEATURE_COLS, bỏ is_trading (constant)
...
TARGET_HORIZON = 5
SEQUENCE_LENGTH = 10          # v1: 20 → v2: 10 (tăng số sample, giảm overfitting)
...
CONFIG_ID = 2            # Cấu hình #2 trong sổ N
N_CONFIGS_TESTED = 12    # Đếm theo số lựa chọn thiết kế thực tế (features, LSTM arch, CB, vol quantiles, L2...)
# Khi N_CONFIGS_TESTED >= 20, threshold tự động nâng lên
...
CB_DRAWDOWN_THRESHOLD_WF = -0.35  # Walk-forward liên tục (T≈1700), E[MDD] lớn hơn → CB lỏng hơn
...
LSTM_UNITS = 32                # v1: 64 → v2: 32
LSTM_LAYERS = 1                # v1: 3 → v2: 1
DROPOUT = 0.15                 # v1: 0.2 → v2: 0.15
...
# --- Feature list (v2 — chỉ ratio/return, không giá tuyệt đối) ---
FEATURE_COLS = [
...
def compute_rsi(series, period=14):
    """Tính RSI trả về giá trị 0-1 (thay vì 0-100) để phù hợp scale."""
    delta = series.diff()
    ...
    rsi = 1.0 - (1.0 / (1.0 + rs))  # 0-1 scale
    return rsi

def add_features(df):
    """
    V2: Feature engineering chỉ dùng ratio/return (cross-ticker comparable).
    Giá tuyệt đối (close, ma, volume) vẫn được tính nhưng chỉ dùng nội bộ
    để tạo ratio, KHÔNG đưa vào FEATURE_COLS.
    """
    df = df.copy()

    # --- Thresholds theo sàn (Buffer +0.01 để tính sai số làm tròn tick size) ---
    # Phân loại dựa trên thực nghiệm quét max_return lịch sử:
    # UPCoM (15%): ORS
    # HNX (10%): APG, BSI, CTS, VDS, VIX
    # HOSE (7%): AGR, FTS, HCM, SSI, TVS
    TICKER_LIMITS = {
        'ORS': 0.15,
        'APG': 0.10, 'BSI': 0.10, 'CTS': 0.10, 'VDS': 0.10, 'VIX': 0.10,
        'AGR': 0.07, 'FTS': 0.07, 'HCM': 0.07, 'SSI': 0.07, 'TVS': 0.07
    }
    BUFFER = 0.01

    # --- Returns ---
    df['return_1d_raw'] = df.groupby('ticker')['close'].pct_change()
    
    # Clip return_1d theo biên độ từng mã (có log khi kích hoạt)
    def clip_by_ticker(group):
        ticker = group.name
        limit = TICKER_LIMITS.get(ticker, 0.15) + BUFFER
        
        # Logging những lần clip kích hoạt (loại bỏ NaN đầu chuỗi)
        overshoots = group[(group.abs() > limit) & (group.notna())]
        if not overshoots.empty:
            for idx, val in overshoots.items():
                date_val = df.loc[idx, 'date'].strftime('%Y-%m-%d')
                print(f"⚠️  CLIP TRIGGER: {ticker} vào {date_val}, gốc={val:.2%}, clip={np.sign(val)*limit:.2%}")
                
        return group.clip(lower=-limit, upper=limit)

    df['return_1d'] = df.groupby('ticker')['return_1d_raw'].transform(clip_by_ticker)
    
    df['return_5d_lag'] = df.groupby('ticker')['close'].pct_change(5)
    df['return_10d_lag'] = df.groupby('ticker')['close'].pct_change(10)

    # --- Moving averages (nội bộ, không cho vào model) ---
    ...
    # --- Ratios (cross-ticker comparable) ---
    df['close_ma5_ratio'] = df['close'] / df['ma5']
    df['close_ma10_ratio'] = df['close'] / df['ma10']
    ...
    # --- Dùng nội bộ cho universe filtering ---
...
def build_lstm(input_shape):
    """V2: 1 layer × 32 units + L2 regularization (~5.5K params)."""
    model = Sequential()
...
def compute_deflated_psr(returns_series, n_configs, benchmark_return=0.0):
    ...
    e_max_sr = compute_expected_max_sr(n_configs, n)
    psr_unadjusted = compute_psr(returns_series, benchmark_return=0.0)
    psr_deflated = compute_psr(returns_series, benchmark_return=e_max_sr)
    return psr_unadjusted, psr_deflated
...
def main():
    print("=" * 70)
    print("PIPELINE V2 — ĐỢT 1 SỬA TOÀN DIỆN")
    print("=" * 70)
    print(f"Thay đổi so với V1:")
    print(f"  Features: {len(FEATURE_COLS)} ratio/return (không giá tuyệt đối)")
    print(f"  LSTM: {LSTM_LAYERS}×{LSTM_UNITS} units + L2={L2_REG}")
    ...
    print("\n===== BẮT ĐẦU 5-FOLD WALK-FORWARD LIÊN TỤC (V2) =====")
    fold_results = []
    current_state = None
    ...
    print(f"\n{'='*50}")
    print(f"Model V2 returns: mean={all_returns.mean():.6f}, std={all_returns.std():.6f}, "
          f"min={all_returns.min():.6f}, max={all_returns.max():.6f}, "
          f"sharpe={(all_returns.mean()/all_returns.std())*np.sqrt(252):.4f}")

    # --- E[MDD] Analysis: σ ex-ante (KHÔNG qua overlay) từ training data ---
    # Ước lượng σ từ dữ liệu train trước fold 1 → độc lập với CB/vol-targeting
    ...
    print(f"\n--- E[MDD] Analysis (Magdon-Ismail: E[MDD] = σ×√(2T/π)) ---")
    print(f"  σ nguồn: ex-ante từ training data (trước {train_end_for_sigma.date()}, {n_est_days} ngày)")
    print(f"           Danh mục {TOP_K_HOLD}-mã equal-weight, KHÔNG vol-targeting/CB/correlation-filter")
    print(f"  ⚠️  Giới hạn đã biết: σ ước lượng từ {DATA_START}→{train_end_for_sigma.date()} cố định")
    print(f"     cho toàn bộ walk-forward (2019→2025). Nếu regime vol thay đổi cấu trúc")
    print(f"     sau 2019, σ có thể không còn đại diện tốt cho giai đoạn sau.")
    print(f"  σ_normal (toàn bộ train)       = {sigma_normal:.6f}")
    print(f"  σ_stress (P{int(0.90*100)} rolling vol)  = {sigma_stress:.6f}")
    print(f"  μ_raw (daily, ex-ante)          = {mu_raw:.6f} ({'dương' if mu_raw >= 0 else 'ÂM'})")
    print(f"  So sánh: model std (sau overlay) = {all_returns.std():.6f} — nhỏ hơn σ_normal {sigma_normal/all_returns.std():.1f}x")
    print()
    print(f"  Driftless check: μ/σ² = {drift_ratio_wf:+.4f} (threshold: |ratio| < {DRIFTLESS_RATIO_THRESHOLD})")
    if status_wf == 'EXACT':
        print(f"    → Driftless approximation: EXACT")
    elif status_wf == 'UPPER_BOUND':
        print(f"    → μ dương đáng kể — driftless là UPPER BOUND (an toàn cho CB justify)")
    else:  # UNSAFE_LOWER_BOUND
        print(f"    → ⛔ μ ÂM đáng kể — driftless là LOWER BOUND, E[MDD] thực tế LỚN HƠN con số dưới đây!")
        print(f"    → CB justification dựa trên con số này KHÔNG AN TOÀN.")
        print(f"    → Cần: Magdon-Ismail bảng tra đầy đủ, hoặc Monte Carlo/bootstrap.")
    print()
    print(f"  Walk-forward (T={T_wf:>4}, σ=σ_normal): E[MDD] = {e_mdd_wf:.4f} ({e_mdd_wf:.1%}) [{status_wf}]")
    print(f"    CB_WF = {CB_DRAWDOWN_THRESHOLD_WF:.0%}", end="")
    if status_wf == 'UNSAFE_LOWER_BOUND':
        print(f" — ⛔ KHÔNG THỂ justify (driftless underestimates risk)")
        print(f"\n⛔ FATAL: Walk-forward E[MDD] check failed. Pipeline STOPPED.")
        return
    elif abs(CB_DRAWDOWN_THRESHOLD_WF) < e_mdd_wf:
        print(f" < E[MDD] → ⚠️  CB sẽ trigger thường xuyên")
        print(f"\n⛔ FATAL: Walk-forward E[MDD] check failed. Pipeline STOPPED.")
        return
    else:
        headroom_wf = (abs(CB_DRAWDOWN_THRESHOLD_WF) - e_mdd_wf) / e_mdd_wf
        print(f" → ✅ headroom {headroom_wf:.0%} trên E[MDD]")
    print()
    # E[MDD] crisis: tính riêng cho từng fold với T thực tế
    print(f"  Crisis folds E[MDD] (σ=σ_stress={sigma_stress:.6f}):")
    for _, test_start_marker, test_end_marker, fold_name, is_prereg in CRISIS_FOLDS:
        ...
        T_crisis = len(trading_days[(trading_days >= t_start) & (trading_days <= t_end)])
        e_mdd_c, status_c, _ = compute_expected_mdd(sigma_stress, T_crisis, mu_raw)
        
        provenance_tag = "" if is_prereg else " (reference-only)"
        print(f"    {fold_name}{provenance_tag} (T={T_crisis}): E[MDD] = {e_mdd_c:.4f} ({e_mdd_c:.1%}) [{status_c}]")
        print(f"      CB_crisis = {CB_DRAWDOWN_THRESHOLD:.0%}", end="")
        if status_c == 'UNSAFE_LOWER_BOUND':
            print(f" — ⛔ KHÔNG THỂ justify")
            print(f"\n⛔ FATAL: Crisis fold {fold_name} E[MDD] check failed. Pipeline STOPPED.")
            return
        elif abs(CB_DRAWDOWN_THRESHOLD) < e_mdd_c:
            print(f" < E[MDD] → ⚠️  CB sẽ trigger thường xuyên")
            print(f"\n⛔ FATAL: Crisis fold {fold_name} E[MDD] check failed. Pipeline STOPPED.")
            return
        else:
            headroom_c = (abs(CB_DRAWDOWN_THRESHOLD) - e_mdd_c) / e_mdd_c
            print(f" → ✅ headroom {headroom_c:.0%}")
    print()
    print(f"  ⚠️  LƯU Ý RỦI RO VỐN THẬT:")
    print(f"     CB_WF = {CB_DRAWDOWN_THRESHOLD_WF:.0%} trên vốn 50M VND "
          f"= mất tối đa {abs(CB_DRAWDOWN_THRESHOLD_WF) * 50:.1f}M VND trước khi cắt.")
    print(f"     CB_crisis = {CB_DRAWDOWN_THRESHOLD:.0%} "
          f"= mất tối đa {abs(CB_DRAWDOWN_THRESHOLD) * 50:.1f}M VND trước khi cắt.")
    print(f"     Đây là quyết định kinh doanh, không chỉ quyết định thống kê.")

    # --- N-REGISTRY: Deflated PSR ---
    psr_model, dsr_model = compute_deflated_psr(all_returns, n_configs=N_CONFIGS_TESTED)
    e_max_sr = compute_expected_max_sr(N_CONFIGS_TESTED, len(all_returns.dropna()))
    print(f"\n--- N-REGISTRY (Config #{CONFIG_ID}, N={N_CONFIGS_TESTED}) ---")
    print(f"PSR (unadjusted):  {psr_model:.4f}")
    print(f"E[max(SR)] under null (N={N_CONFIGS_TESTED}): {e_max_sr:.6f}")
    ...
    print(f"\n{'='*60}")
    print("BẢNG SO SÁNH V1 vs V2")
    print(f"{'='*60}")
    print(f"{'Metric':<25} {'V1 (cũ)':<15} {'V2 (mới)':<15}")
    print(f"{'-'*55}")
    print(f"{'Features':<25} {'14 (giá thô)':<15} {f'{len(FEATURE_COLS)} (ratio)':<15}")
    print(f"{'LSTM params':<25} {'~50K':<15} {'~5.5K':<15}")
    ...
    if binding_psr >= PSR_THRESHOLD and psr_model > psr_baseline and covid_passed:
        print(f">>> KẾT LUẬN: Mô hình V2 có tín hiệu.")
        ...
        print(f">>> KẾT LUẬN: Mô hình V2 KHÔNG đạt tín hiệu.")
        ...
        if psr_model > 0.6640:
            print(f"    PSR cải thiện từ 0.6640 (V1) lên {psr_model:.4f} (V2), nhưng chưa đạt threshold.")
