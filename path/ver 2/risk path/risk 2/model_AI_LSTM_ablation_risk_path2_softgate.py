...
VERSION 2 — ĐỢT 1 SỬA TOÀN DIỆN:
  ...
  7. Lối ra 2 (Bỏ E[MDD] driftless làm gate chặn cứng, chuyển thành soft-gate)
  Thay đổi: Đã loại bỏ lệnh return dừng hoàn toàn pipeline (FATAL) mỗi khi mô hình chạm ngưỡng E[MDD].
  Việc chạm ngưỡng giờ đây chỉ in ra dòng log ⚠️ CẢNH BÁO...
  Chuyển thành soft-gate để làm thông tin tham khảo (đúng theo tiêu chí đơn giản, ít giả định sai hơn).
"""
...
def main():
    ...
    if status_wf == 'UNSAFE_LOWER_BOUND':
        ...
        print(f"\n⚠️ CẢNH BÁO: Walk-forward E[MDD] check failed. Chuyển thành soft-gate, tiếp tục pipeline.")
    ...
    for _, test_start_marker, test_end_marker, fold_name, is_prereg in CRISIS_FOLDS:
        ...
        if status_c == 'UNSAFE_LOWER_BOUND':
            ...
            print(f"\n⚠️ CẢNH BÁO: Crisis fold {fold_name} E[MDD] check failed. Chuyển thành soft-gate, tiếp tục pipeline.")
