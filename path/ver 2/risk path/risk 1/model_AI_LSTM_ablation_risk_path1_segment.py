...
VERSION 2 — ĐỢT 1 SỬA TOÀN DIỆN:
  ...
  7. Lối ra 1 (Đo E[MDD] theo từng đoạn phơi nhiễm):Thay đổi: Tôi đã thêm logic log lại trạng thái
in_cash trong hàm run_backtest. Sau đó, ở phần tính E[MDD] cho walk-forward,
mã sẽ tự động tìm đoạn phơi nhiễm dài nhất (max segment) khi danh mục không nằm ở trạng thái cash,
tính T_max_segment thay vì gộp toàn bộ T_wf = 1744
"""
...
def run_backtest(df_features, model, scaler, tickers_all, start_date, end_date, trading_days,
    ...
    res = pd.DataFrame({'date': sim_days, 'daily_return': portfolio_returns, 'in_cash': in_cash_log})
    final_state = {
        'holdings': holdings,
...
def main():
    ...
    # --- Path 1: Tính T_max_segment ---
    in_cash_series = pd.concat([ret_df['in_cash'] for ret_df, _, _, _ in fold_results], ignore_index=True)
    is_exposed = ~in_cash_series
    segment_ids = (is_exposed != is_exposed.shift()).cumsum()
    exposure_segments = is_exposed.groupby(segment_ids).sum()
    exposure_segments = exposure_segments[exposure_segments > 0]
    T_max_segment = int(exposure_segments.max()) if len(exposure_segments) > 0 else 1
    print(f"\n{'='*50}")
    ...
    e_mdd_wf, status_wf, drift_ratio_wf = compute_expected_mdd(sigma_normal, T_max_segment, mu_raw)

    print(f"\n--- E[MDD] Analysis (Magdon-Ismail: E[MDD] = σ×√(2T/π) ĐO THEO SEGMENT) ---")
    ...
    print()
    print(f"  Walk-forward (T_segment={T_max_segment:>4}, T_total={T_wf}, σ=σ_normal): E[MDD] = {e_mdd_wf:.4f} ({e_mdd_wf:.1%}) [{status_wf}]")
