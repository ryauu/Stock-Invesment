...
=================================================================
VERSION 2 (ABLATION MODEL - Option 1: Loose Contract) — Giữ nguyên Features/LSTM V1:
...
CONFIG_ID = 4            # Cấu hình #2 trong sổ N
N_CONFIGS_TESTED = 15    # Đếm theo số lựa chọn thiết kế thực tế (features, LSTM arch, CB, vol quantiles, L2...)
...
GLOBAL_MDD_KILL_SWITCH = -0.50
CB_RECOVERY_RSI_THRESHOLD = 35
...
CB_DRAWDOWN_THRESHOLD_WF = -0.35  # Ngưỡng kích hoạt bán ra (Trigger)
CB_DESIGN_CONTRACT = -0.40      # Lời hứa tối đa (Contract Gate)
...
def run_backtest(df_features, model, scaler, tickers_all, start_date, end_date, trading_days,
    ...
    if initial_state is None:
        ...
        global_peak_value = initial_state.get('global_peak_value', 1.0)
        ...
        exposure_scale = initial_state.get('exposure_scale', 1.0)
        recovery_days = initial_state.get('recovery_days', 0)
        cb_cooldown_days = initial_state.get('cb_cooldown_days', 0)
        re_entry_portfolio_value = initial_state.get('re_entry_portfolio_value', 1.0)
    ...
    for i, date in enumerate(sim_days):
        ...
        # Giảm cooldown
        if in_cash and cash_reason == 'circuit_breaker' and cb_cooldown_days > 0:
            cb_cooldown_days -= 1
            
        # Kiểm tra phục hồi nếu đang trong cash do circuit breaker và đã hết cooldown
        if in_cash and cash_reason == 'circuit_breaker' and cb_cooldown_days == 0:
            ...
            # Tính average RSI và kiểm tra MA20 trend
            rsis = []
            yesterday_rsis = []
            returns = []
            above_ma20_count = 0
            ...
            for tic in universe_now:
                sub = df_features[(df_features['ticker'] == tic) & (df_features['date'] <= date)].tail(2)
                if not sub.empty:
                    today_row = sub.iloc[-1]
                    if not np.isnan(today_row.get('rsi_14', np.nan)):
                        rsis.append(today_row['rsi_14'])
                    if len(sub) == 2 and not np.isnan(sub.iloc[0].get('rsi_14', np.nan)):
                        yesterday_rsis.append(sub.iloc[0]['rsi_14'])
                        
                    if not np.isnan(today_row.get('return_1d', np.nan)):
                        returns.append(today_row['return_1d'])
                        
                    if not np.isnan(today_row.get('ma20', np.nan)) and not np.isnan(today_row['close']):
                        valid_trend_count += 1
                        if today_row['close'] > today_row['ma20']:
                            above_ma20_count += 1
                            
            avg_rsi = np.mean(rsis) if rsis else 50.0
            avg_yesterday_rsi = np.mean(yesterday_rsis) if yesterday_rsis else 50.0
            avg_return = np.mean(returns) if returns else 0.0
            trend_ratio = (above_ma20_count / valid_trend_count) if valid_trend_count > 0 else 0.0
            
            is_uptrend = trend_ratio > 0.5
            is_green_candle = avg_return > 0
            rsi_crossed_up = (avg_yesterday_rsi < CB_RECOVERY_RSI_THRESHOLD) and (avg_rsi > CB_RECOVERY_RSI_THRESHOLD)
            
            # Điều kiện phục hồi chữ V: (RSI < 35 và là nến xanh) HOẶC (RSI cắt lên 35)
            is_v_shape = (avg_rsi < CB_RECOVERY_RSI_THRESHOLD and is_green_candle) or rsi_crossed_up
            
            # Điều kiện phục hồi: Hoặc (vol thấp VÀ uptrend) HOẶC (V-shape oversold xác nhận)
            if (current_vol < vol_threshold_recovery and is_uptrend) or is_v_shape:
                reason_str = "V-SHAPE Xác nhận" if is_v_shape else f"VOL < THRESH & UPTREND ({trend_ratio:.0%} > MA20)"
                print(f"    [DIAGNOSTIC] {date.date()}: 🟢 CB RECOVERY! {reason_str}")
                in_cash = False
                cash_reason = None
                peak_value = cash_balance  # Reset peak value to prevent instant re-triggering
                exposure_scale = 0.3  # Bắt đầu giải ngân 30%
                recovery_days = 0
                re_entry_portfolio_value = cash_balance
                force_rebalance = True
            elif date in rebalance_dates:
                print(f"    [DIAGNOSTIC] {date.date()}: 🔴 CB LOCKED. vol={current_vol:.4f}, uptrend={is_uptrend}, rsi={avg_rsi:.1f}")
        ...
        if current_value > global_peak_value:
            global_peak_value = current_value
            
        cb_triggered_today = False

        # 2a. Kiểm tra Kill Switch toàn cục (Global Drawdown)
        if not in_cash and cash_reason != 'kill_switch':
            global_dd = (current_value / global_peak_value) - 1
            if global_dd <= GLOBAL_MDD_KILL_SWITCH:
                print(f"    [DIAGNOSTIC] {date.date()}: 💀 GLOBAL KILL SWITCH TRIGGERED! Drawdown {global_dd:.2%} <= {GLOBAL_MDD_KILL_SWITCH:.2%}.")
                fee_cost = current_value * fee
                current_value -= fee_cost
                cash_balance = current_value
                holdings = {}
                in_cash = True
                cash_reason = 'kill_switch'
                exposure_scale = 0.0
                turnover_log.append((date, 1.0))
                cb_triggered_today = True

        # 2b. Kiểm tra circuit breaker thông thường
        if not in_cash and not cb_triggered_today and cash_reason != 'kill_switch':
        ...
        # Cập nhật số ngày phục hồi và nâng tỷ trọng (DCA) có điều kiện
        if not in_cash and not cb_triggered_today and cash_reason != 'kill_switch':
            if exposure_scale > 0.0 and exposure_scale < 1.0:
                recovery_days += 1
                
                # Điều kiện nâng tỷ trọng: Đang có lãi (current_value > re_entry) HOẶC thị trường đã uptrend
                is_profitable = current_value > re_entry_portfolio_value
                
                # Tính uptrend (VNINDEX hoặc universe)
                universe_now = filter_universe(df_features, date, trading_days, top_frac)
                above_ma20_count = 0
                valid_trend_count = 0
                for tic in universe_now:
                    row = df_features[(df_features['ticker'] == tic) & (df_features['date'] == date)]
                    if not row.empty and not np.isnan(row.iloc[0].get('ma20', np.nan)) and not np.isnan(row.iloc[0]['close']):
                        valid_trend_count += 1
                        if row.iloc[0]['close'] > row.iloc[0]['ma20']:
                            above_ma20_count += 1
                trend_ratio = (above_ma20_count / valid_trend_count) if valid_trend_count > 0 else 0.0
                is_uptrend = trend_ratio > 0.5

                can_scale_up = is_profitable or is_uptrend

                if recovery_days == 10:
                    if can_scale_up:
                        print(f"    [DIAGNOSTIC] {date.date()}: 🚀 Nâng tỷ trọng 100% (sau 10 ngày, lãi/uptrend=True)")
                        exposure_scale = 1.0
                        force_rebalance = True
                    else:
                        print(f"    [DIAGNOSTIC] {date.date()}: ⏸️ Tạm hoãn nâng 100% (chưa đủ điều kiện)")
                        recovery_days -= 1 # Giữ nguyên số ngày để chờ
                elif recovery_days == 5:
                    if can_scale_up:
                        print(f"    [DIAGNOSTIC] {date.date()}: 📈 Nâng tỷ trọng 60% (sau 5 ngày, lãi/uptrend=True)")
                        exposure_scale = 0.6
                        force_rebalance = True
                    else:
                        print(f"    [DIAGNOSTIC] {date.date()}: ⏸️ Tạm hoãn nâng 60% (chưa đủ điều kiện)")
                        recovery_days -= 1 # Giữ nguyên số ngày để chờ

        # 3. Rebalance (nếu không dính CB hôm nay và đủ điều kiện)
        allow_rebalance = (not in_cash) or (cash_reason == 'no_candidates')
        do_rebalance = ((date in rebalance_dates) or force_rebalance) and allow_rebalance and not cb_triggered_today
        ...
        if do_rebalance:
            ...
            # Áp dụng exposure_scale từ DCA re-entry
            invest_frac = invest_frac * exposure_scale
    ...
    final_state = {
        ...
        'cb_cooldown_days': cb_cooldown_days,
        're_entry_portfolio_value': re_entry_portfolio_value
    }
