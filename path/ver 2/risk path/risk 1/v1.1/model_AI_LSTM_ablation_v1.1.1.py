...
=================================================================
VERSION 2 (ABLATION MODEL) — Giữ nguyên Features/LSTM V1, Áp dụng Risk Mgmt V2:
...
N_CONFIGS_TESTED = 14    # Đếm theo số lựa chọn thiết kế thực tế (features, LSTM arch, CB, vol quantiles, L2...)
...
def main():
    ...
    for seg_id, group in is_exposed_series.groupby(segment_ids):
        if group.iloc[0]: # Nếu đang exposed (is_exposed == True)
            ...
            print(f"  Episode [{d_start} to {d_end}]: Length = {len(seg_returns):4d} days, MDD = {ep_mdd:.2%}")
    ...
    print(f"  CB_WF design contract = {CB_DRAWDOWN_THRESHOLD_WF:.0%}")
    
    if episode_mdd_max < CB_DRAWDOWN_THRESHOLD_WF:
        print(f"  → ⛔ FATAL: MDD một chu kỳ ({episode_mdd_max:.1%}) VƯỢT QUÁ CB_WF ({CB_DRAWDOWN_THRESHOLD_WF:.0%})")
        print(f"     CB đã thất bại trong việc cắt lỗ kịp thời đúng lời hứa. Pipeline STOPPED.")
        return
    else:
        headroom = abs(CB_DRAWDOWN_THRESHOLD_WF) - abs(episode_mdd_max)
        print(f"  → ✅ CB hoạt động đúng thiết kế. Headroom = {headroom:.1%}")
