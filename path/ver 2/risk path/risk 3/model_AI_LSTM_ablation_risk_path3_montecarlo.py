...
VERSION 2 — ĐỢT 1 SỬA TOÀN DIỆN:
  ...
  7. Lối ra 3 (Chuyển sang Monte Carlo / Bootstrap)
  Thay đổi: Hàm compute_expected_mdd đã được viết lại bằng thuật toán mô phỏng Bootstrap (Monte Carlo)
  với 10,000 kịch bản ngẫu nhiên dựa trên phân phối sinh ra từ μ và σ của tham số.
  Đây là hướng chính xác nhất với thực tế và không bị phụ thuộc vào giả định driftless của Magdon-Ismail.
"""
...
CONFIG_ID = 3            # Cấu hình #2 trong sổ N
N_CONFIGS_TESTED = 13    # Đếm theo số lựa chọn thiết kế thực tế (features, LSTM arch, CB, vol quantiles, L2...)
...
def compute_expected_mdd_mc(sigma_daily, T_days, mu_daily=0.0, n_paths=10000):
    """
    Monte Carlo bootstrap cho E[MDD].
    Mô phỏng n_paths kịch bản giá với mu_daily và sigma_daily.
    """
    if T_days <= 1:
        return 0.0, 'MC_EXACT', mu_daily/(sigma_daily**2 + 1e-9) if sigma_daily > 0 else float('inf')
    
    rets = np.random.normal(loc=mu_daily, scale=sigma_daily, size=(n_paths, T_days))
    cum_returns = np.cumprod(1 + rets, axis=1)
    
    rolling_max = np.maximum.accumulate(cum_returns, axis=1)
    drawdowns = (cum_returns / rolling_max) - 1
    max_dds = np.min(drawdowns, axis=1)
    
    e_mdd = np.abs(np.mean(max_dds))
    drift_ratio = mu_daily / (sigma_daily ** 2) if sigma_daily > 0 else float('inf')
    
    return e_mdd, 'MC_SIMULATED', drift_ratio
...
def main():
    ...
    e_mdd_wf, status_wf, drift_ratio_wf = compute_expected_mdd_mc(sigma_normal, T_wf, mu_raw)

    print(f"\n--- E[MDD] Analysis (Monte Carlo Simulation, N=10000) ---")
    ...
    for _, test_start_marker, test_end_marker, fold_name, is_prereg in CRISIS_FOLDS:
        ...
        e_mdd_c, status_c, _ = compute_expected_mdd_mc(sigma_stress, T_crisis, mu_raw)
